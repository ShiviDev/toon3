...
contract Creator {
    ...
    modifier userAlreadyHasAccount() {
        require(
            keccak256(abi.encodePacked(users[msg.sender].name)) ==
    ...
    modifier userDoesNotHaveAccount(address _userAddress) {
        require(
            keccak256(abi.encodePacked(users[_userAddress].name)) !=
                keccak256(abi.encodePacked("")),
    ...
    modifier userHasDonations(address _creatorAdddress, uint _amount) {
        require(
            balances[_creatorAdddress] <= _amount,
            "You do not have an account!"
        );
        _;
    }
    ...
    function subscribeToUser(
        address _creator
    ) public alreadySubscribed(_creator) {
    ...
    function withdrawDonations(
        ...
        uint _amount
    ) public payable userHasDonations(_creatorAddress, _amount) {
    ...
    function getBookDetails(
        uint256 _bookId
    ) public view returns (bookMetadata memory) {
    ...
    function addChapter(uint256 _bookId, string[] memory _CIDR) public {
        ...
        books[_bookId].chapters[
            books[_bookId].totalChapters + 1
        ] = chapterMetadata({
            chapterNumber: books[_bookId].totalChapters + 1,
            imageCIDR: _CIDR
        });
        books[_bookId].totalChapters += 1;
